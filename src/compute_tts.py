...
def crop_rast_to_country(rast_path, admin0_gdf, country_num:str, savepath, is_pop = False):
    """
    ...
    archive, file = savepath.replace('zip://', '').split('!')
    archivepath = pathlib.Path(archive)
    savedir = archivepath.parent

    os.makedirs(savedir, exist_ok=True)


    with tempfile.TemporaryDirectory() as tmpdir:
        tmppath = os.path.join(tmpdir, 'cropped.geotiff')
        
        with rasterio.open(tmppath, "w", **cropped) as dest:
            dest.write(clipped_rast)
        
        with zipfile.ZipFile(archivepath, 'a') as zfile:
            zfile.write(tmppath, arcname=file)
    ...
    country_rast_cropped = rasterio.open(savepath, 'r')

    pop_arr = country_rast_cropped.read()[0]
    country_rast_cropped.close()
    return(savepath, pop_arr)
...
def transform_to_utm(input_raster, output_raster, dst_crs):
    ...
    with rasterio.open(input_raster) as src:
        ...
        archive, file = output_raster.replace('zip://', '').split('!')
        archivepath = pathlib.Path(archive)
        savedir = archivepath.parent
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with rasterio.open(tmppath, 'w', **kwargs) as dst:
