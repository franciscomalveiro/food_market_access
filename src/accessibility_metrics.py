...
# CHANGED HERE: necessary to get filename without extension
import pathlib
...
def shortest_travel_time(tt_rast_path):
    ...
    """
    i = 0
    if len(os.listdir(tt_rast_path)) == 0:
        raise ValueError(f"No files in {tt_rast_path}")
    # CHANGED HERE
    # make sure the files are listed in order (sorted names)
    for file in tqdm(sorted(os.listdir(tt_rast_path)), desc='Computing shortest travel time'):
        ...
        if file.endswith('.tif') or file.endswith('.geotiff'):
            ...
            if i == 0:
            # CHANGED HERE: create a np.inf array in the beginning to catch unacessed points
                
                min_raster = np.full(point_tt_arr.shape, np.inf)
...
# CHANGED HERE: introduce function to obtain index of closest facility
# copy and adaptation from above
def closest_facility(tt_rast_path):
    if len(os.listdir(tt_rast_path)) == 0:
        raise ValueError(f"No files in {tt_rast_path}")
    ...
    for file in tqdm(sorted(os.listdir(tt_rast_path)), desc='Finding closest facility'):
        if file.endswith('.tif') or file.endswith('.geotiff'):
            fac_idx = pathlib.Path(file).stem 
            ...
            if idx == 0:
                # create empty array
                # these lines are here and not outside to be able to use the size of the array
                min_raster = np.full(point_tt_arr.shape, np.inf)
                min_idx_raster = np.full(point_tt_arr.shape, np.nan)

                
                #min_raster = point_tt_arr.copy()
                #min_idx_raster = np.full(point_tt_arr.shape, fac_idx) # point_tt_arr.copy()
            #else:
            min_idx_raster = np.where(point_tt_arr < min_raster, fac_idx, min_idx_raster)
            min_raster = np.fmin(point_tt_arr, min_raster)
            idx += 1
    ...
    return min_idx_raster.astype(float)
